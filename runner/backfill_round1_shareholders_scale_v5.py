...
# -*- coding: utf-8 -*-
"""
Round 1.1 + R1.2: 十大流通股东 + 公司规模信息 — 修复版（subprocess curl + timeout）
...
def fetch_json(url: str) -> dict:
    try:
        ...
        if result.returncode != 0:
            logger.debug(f"curl failed: {result.returncode}")
            return {}
...
def load_remaining_companies():
    with engine.connect() as conn:
        done_sh = {r[0] for r in conn.execute(text("SELECT DISTINCT company_id FROM company_shareholders")).fetchall()}
        rows = conn.execute(text("""
            SELECT id, ticker, exchange, company_name, registered_capital, employee_count
            FROM companies WHERE is_active = true ORDER BY id
        """)).fetchall()
    remaining = [r for r in rows if r.id not in done_sh]
    logger.info(f"Total: {len(rows)}, Shareholders done: {len(done_sh)}, Remaining for R1.1: {len(remaining)}")
...
def backfill_shareholders(companies):
    ...
    total = len(companies)
    if total == 0:
        logger.info("R1.1: All companies already have shareholders. Skipping.")
        return 0, 0, 0
    ...
    for idx, row in enumerate(companies, 1):
        ...
        try:
            ...
            data = None
            for d in dates:
                url = f"https://emweb.securities.eastmoney.com/PC_HSF10/ShareholderResearch/PageSDGD?code={em_code}&date={d}"
                resp = fetch_json(url)
...
def backfill_company_scale(companies):
    ...
    for idx, row in enumerate(companies, 1):
        ...
        em_code = build_em_code(ticker, row.exchange)
        company_id = row.id
        needs_update = row.registered_capital is None or row.employee_count is None
        if not needs_update:
            skipped += 1
            continue
...
def main():
    all_companies, remaining = load_remaining_companies()

    logger.info("=" * 60)
    logger.info("START R1.1: Company Shareholders (resume)")
    logger.info("=" * 60)
    backfill_shareholders(remaining)

    logger.info("=" * 60)
    logger.info("START R1.2: Company Scale Info")
    logger.info("=" * 60)
    backfill_company_scale(all_companies)
