...
# -*- coding: utf-8 -*-
"""
Round 1.1 + R1.2: 十大流通股东 + 公司规模信息 — 修复版（断点恢复）
...
def fetch_json(url: str, timeout: tuple = (5, 10)) -> dict:
    try:
...
def backfill_shareholders(companies, all_companies):
    """R1.1: 十大流通股东"""
    total = len(companies)
    if total == 0:
...
def main():
    ...
    logger.info("=" * 60)
    backfill_shareholders(remaining, all_companies)
